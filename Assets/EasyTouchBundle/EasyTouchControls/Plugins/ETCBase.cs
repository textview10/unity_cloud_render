...
public abstract class ETCBase : MonoBehaviour {
	...
	public virtual void LateUpdate(){
		if (enableCamera){
			Debug.Log("wangxuTest = " + cameraLookAt.gameObject.name );
			// find camera 
	...
	protected void CameraSmoothFollow(){
		...
		if (enableWallDetection){
			...
			if (Physics.Linecast( new Vector3(cameraLookAt.position.x,cameraLookAt.position.y+1f,cameraLookAt.position.z),newPos, 
				out wallHit, wallLayer)){
